...
const StepTwo = ({ setActive }) => {
    ...
    return (
        <div className="flex flex-col items-start justify-center">
            ...
            <p className="text-2xl text-gray-400 text-center mb-10">Choose your favorite template</p>
            <div className='grid grid-cols-3 gap-4'>
                {TEMPLATES.map((template) => <TemplateCard key={template.id} data={template} selected={selected} setSelected={setSelected} />)}
            </div>
            <button onClick={() => setActive(2)} className='btn btn-wide mt-6'>Next</button>
        </div>
    )
}
